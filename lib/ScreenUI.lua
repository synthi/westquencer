...
function ScreenUI.set_patching_view(src, dst, is_conn) 
  ScreenUI.patching_view.active=true
  ScreenUI.patching_view.src=src
  ScreenUI.patching_view.dst=dst
  ScreenUI.patching_view.connected=is_conn
  ScreenUI.dirty=true 
end

function ScreenUI.clear_patching_view() 
  ScreenUI.patching_view.active=false
  ScreenUI.dirty=true 
end

function ScreenUI.open_context_menu(id, is_in, is_out) 
  ScreenUI.context.active=true
  ScreenUI.context.id=id
  ScreenUI.context.is_input=is_in
  ScreenUI.context.is_output=is_out
  ScreenUI.dirty=true 
end

function ScreenUI.close_context_menu() 
  ScreenUI.context.active=false
  ScreenUI.dirty=true 
end

function ScreenUI.trigger_message(msg) 
  ScreenUI.popup.name=msg
  ScreenUI.popup.value=""
  ScreenUI.popup.active=true
  ScreenUI.popup.deadline=util.time()+1.0
  ScreenUI.dirty=true 
end

function ScreenUI.handle_key(n, z) 
  if z==1 then 
    ScreenUI.popup.name="KEY "..n
    ScreenUI.popup.value="PRESS"
    ScreenUI.popup.active=true
    ScreenUI.popup.deadline=util.time()+1.5 
  end 
end
  ...
  if ScreenUI.context.is_input and PatchbayRef then
     ...
     if conns and #conns > 0 then
        ...
        for k, src_id in ipairs(conns) do
           ...
           local name = LogicOps.NAMES[src_id] or ("SRC "..src_id); if src_id >= 100 then name = "STEP "..(src_id-100) end
...
function ScreenUI.draw_view()
  if ScreenUI.page == 2 then ScreenUI.draw_settings_page(); return end
...
function ScreenUI.draw_scope()
  screen.level(15); screen.move(0, 8); screen.font_size(8); screen.text("WESTQUENCER v0.59.1")
...
function ScreenUI.draw_editor_overlay() 
  ...
  local indices = {}
  for idx, _ in pairs(EngineRef.selection.steps) do table.insert(indices, idx) end
  table.sort(indices)
  
  for _, idx in ipairs(indices) do
     if count < 5 then steps_txt = steps_txt .. idx .. ", " end
     count = count + 1
     last_val = EngineRef.steps[idx].vals[EngineRef.selection.row_id]
  end
  ...
  screen.move(64, 15); screen.font_size(8); screen.text_center("EDITING STEPS: " .. steps_txt)
  screen.move(10, 40); screen.font_size(16); screen.text("ROW " .. EngineRef.selection.row_id)
  screen.move(118, 40); screen.text_right(tostring(last_val))
...
function ScreenUI.draw_popup_overlay() 
  if util.time() > ScreenUI.popup.deadline then 
    ScreenUI.popup.active = false
    ScreenUI.dirty = true
    return 
  end
